...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchBtn.addEventListener('click', function() {
        ...
        if (searchType === 'reference') {
            ...
            // Redirect to LESCO bill page based on bill type
            const billPage = billType === 'U' ? 'general' : 'industrial';
            window.location.href = `https://bill.pitc.com.pk/lescobill/${billPage}?refno=${billNumber}`;
        } else {
            // Customer ID validation (7 digits)
            if (billNumber.length !== 10) {
                errorDiv.textContent = 'Customer ID must be 10 digits';
                errorDiv.style.display = 'block';
                return;
            }
            // Redirect to LESCO bill page with customer ID
            window.location.href = `https://bill.pitc.com.pk/lescobill/general?custid=${billNumber}`;
        }
    });
});